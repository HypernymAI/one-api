  ...
  return (
    <>
      <Segment loading={loading}>
        <Header as='h3'>{isEdit ? 'Update Redemption Code Information' : 'Create New Redemption Code'}</Header>
        <Form autoComplete='new-password'>
          ...
          <Form.Field>
            <Form.Input
              label={`Balance${renderQuotaWithPrompt(quota)}`}
              name='quota'
              placeholder={'Please enter the balance contained in a single redemption code'}
              onChange={handleInputChange}
              value={quota}
              autoComplete='new-password'
              type='number'
            ...
            !isEdit && <>
              <Form.Field>
                <Form.Input
                  ...
                  name='count'
                  placeholder={'Please enter quantity to generate'}
                  onChange={handleInputChange}
                  value={count}
                  autoComplete='new-password'
                  type='number'
